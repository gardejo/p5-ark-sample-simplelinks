...
is_deeply( [$site_b->category_ids], [],
           '$site_b->category_ids [] ok' );
...
is_deeply( [$site_h->category_ids], [$cat_e->id],
           '$site_h->category_ids [e] ok' );

# $website->categories
is_deeply( [map { $_->id } $site_a->categories], [$cat_b->id],
           '$site_a->categories [a] ok' );
is_deeply( [map { $_->id } $site_b->categories], [],
           '$site_b->categories [] ok' );
...
is_deeply( [map { $_->id } $site_h->categories], [$cat_e->id],
           '$site_h->categories [e] ok' );
...
is( $cat_b->count_websites, 0, '$cat_b->count_websites [0] [1] ok' );
...
is( $cat_g->count_websites, 0, '$cat_g->count_websites [0] [1] ok' );

# $category->count_current_websites
is( $cat_a->count_current_websites, 0, '$cat_a->count_websites [0]<[0] ok' );
is( $cat_b->count_current_websites, 1, '$cat_b->count_websites [1]<[1] ok' );
...
is( $cat_g->count_current_websites, 1, '$cat_g->count_websites [1]<[1] ok' );

# $category->count_websites (retry)
is( $cat_a->count_websites, 0, '$cat_a->count_websites [0] [0] ok' );
is( $cat_b->count_websites, 1, '$cat_b->count_websites [1] [1] ok' );
is( $cat_d->count_websites, 1, '$cat_d->count_websites [1] [1] ok' );
is( $cat_e->count_websites, 2, '$cat_e->count_websites [2] [2] ok' );
is( $cat_f->count_websites, 0, '$cat_f->count_websites [0] [0] ok' );
is( $cat_g->count_websites, 1, '$cat_g->count_websites [1] [1] ok' );
